...
class GUI:
    ...
    def __init__(self):
        ...
        Tenv_label = ttk.Label(self.root, text="Temperatura ambiente:", font=("Arial", 16))
        Tenv_label.grid(row=6, column=0, padx=5, pady=5, sticky=tk.E)
        self.Tenv_entry = ttk.Entry(self.root, font=("Arial", 16), width=10)
        self.Tenv_entry.grid(row=6, column=1, padx=5, pady=5)
    ...
    def T(self):
        ...
        for counter in range(1, int(self.numDots)):
            T1.append(   ( -(T1[len(T1) - 1]  - T2[len(T2) - 1])/(self.R * self.m1 * self.c1) - (T1[len(T1) - 1]  - self.Tenv)/(self.R * self.m1 * self.c1)) * dt + T1[len(T1) - 1]   )
            T2.append(   ( -(T2[len(T2) - 1]  - T1[len(T1) - 1])/(self.R * self.m2 * self.c2) - (T2[len(T2) - 1]  - self.Tenv)/(self.R * self.m2 * self.c2)) * dt + T2[len(T2) - 1]   )
